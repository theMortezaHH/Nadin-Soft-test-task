...
import data from "@/database/location-data.json"
import weathercodes from "@/database/weathercodes.json"
...
function onSearch() {
    ...
    const currectInputType =
        cityNameInput.value.charAt(0).toUpperCase() + cityNameInput.value.slice(1)

    //this const filters city names
    const avalableOptions = cityNames.value.filter((x) => x.includes(currectInputType))
